...
public class ResourcesController extends BaseController {
	...
	@ResponseBody
	@RequestMapping("treelists2")
	public JSONObject findByPage2(Model model) {
		ResFormMap resFormMap = getFormMap(ResFormMap.class);
		String order = " order by level asc";
		resFormMap.put("$orderby", order);
		List<ResFormMap> mps = resourcesMapper.findByNames(resFormMap);
		...
		TreeUtil treeUtil = new TreeUtil();
		List<TreeObject> ns = treeUtil.getChildTreeObjects(list, 0);
		TreeObject tr = new TreeObject();
		tr.setId(0);
		tr.setName("菜单");
		tr.setChildren(ns);
		JSONObject ja = JSONObject.fromObject(tr);
		String jsonStr = ja.toString().replace("name", "text");
