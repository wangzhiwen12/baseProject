				...
				<div class="zd" style="text-align: center;">
					...
					<div>
						<ul class="list-unstyled">
							<li>
								<a href="#" id="pic_add" class="btn btn-info btn-lg" style="width: 200px;margin-top: 20px;">
							...
							<li>
								<button type="button" onclick="cententPic('1');">abcd</button>
								<button type="button" onclick="picSwitch();">efgh</button>
							</li>
						</ul>
					</div>
				</div>
...
function picSwitch(){
	var picList = $("img[name='pic']");
	//alert(picList[0].src);
	for(var n = 0;n < picList.length;n++){
		var img = null;
		img = picList[n].src;
		picList[n].src = picList[n].alt;
		picList[n].alt = img;
	}
}
function cententPic(map){
	//var content = $('#editor').code();
	//var markupStr = '<img name="pic" src="'+map.localUrl+'" alt="'+map.url+'" >';
...
$(function() {
	...
	$('#material_upload').click(function() {
		pageii = layer.open({
					...
					type : 2,
					area : [ "50%", "50%" ],
					content : '${pageContext.request.contextPath}/materialLocal/imageInsert.shtml'
				});
	});
	$('#pic_add').click(function() {
		pageii = layer.open({
					...
					type : 2,
					area : [ "50%", "50%" ],
					content : '${pageContext.request.contextPath}/materialLocal/imgUrlAdd.shtml'
				});
	});
	...
	$('#news_add_btn').click(
		function() {
			var localContent = $('#editor').code();//本地存储的内容
			picSwitch();
			...
			$.ajax({
				...
				url : '${pageContext.request.contextPath}/materialLocal/articleAdd.shtml',
